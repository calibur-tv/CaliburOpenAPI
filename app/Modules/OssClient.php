...
class OssClient
{
     ...
     * @return int
     */
    public function computePartSize($partSize)
    {
     ...
     * @throws OssException
     */
    public function precheckOptions(&$options)
    {
     ...
     * @throws OssException
     */
    public function precheckBucket($bucket, $errMsg = 'bucket is not allowed empty')
    {
     ...
     * @throws OssException
     */
    public function precheckObject($object)
    {
     ...
     * @throws OssException
     */
    public function precheckStorage($storage)
    {
     ...
     * @throws OssException
     */
    public function precheckParam($options, $param, $funcName)
    {
        if (!isset($options[$param])) {
     ...
     * @return bool|null
     */
    public function isCheckMD5($options)
    {
     ...
     * @return bool|null
     */
    public function getValue($options, $key, $default = NULL, $isCheckEmpty = false, $isCheckBool = false)
    {
        $value = $default;
    ...
    public function auth($options)
    {
        ...
        $headersArray['User-Agent'] = $this->generateUserAgent();
        // Streaming uploads
        ...
        if (isset($options[self::OSS_CONTENT])) {
            $requestParam['body'] = $options[self::OSS_CONTENT];
        ...
        foreach ($headers as $header_key => $header_value) {
            ...
            if ($header_value !== '' || $header_key === self::OSS_ACCEPT_ENCODING) {
                $headersArray[$header_key] = $header_value;
            }
        ...
        $headersArray['Authorization'] = 'OSS ' . $this->accessKeyId . ':' . $signature;
        $requestParam['headers'] = $headersArray;

        try {
            $response = $requestClient->request('PUT', $this->requestUrl, $requestParam);
        } catch (\Exception $e) {
        ...
        $response_header['oss-requestheaders'] = $headersArray;

        $data = new ResponseCore($response_header, $response->getBody(), $response->getStatusCode());
     ...
     * @throws OssException
     */
    public function authPrecheckBucket($options)
    {
     ...
     * @throws OssException
     */
    public function authPrecheckObject($options)
    {
     ...
     */
    public function authPrecheckObjectEncoding(&$options)
    {
     ...
     * @throws OssException
     */
    public function authPrecheckAcl($options)
    {
     ...
     */
    public function generateHostname($bucket)
    {
     ...
     */
    public function generateResourceUri($options)
    {
        $resource_uri = "";
     ...
     * @return array
     */
    public function generateSignableQueryStringParam($options)
    {
     ...
     * @return string
     */
    public function generateSignableResource($options)
    {
        $signableResource = "";
        $signableResource .= '/';
        if (isset($options[self::OSS_BUCKET]) && '' !== $options[self::OSS_BUCKET]) {
    ...
    public function stringToSignSorted($string_to_sign)
    {
        $queryStringSorted = '';
     ...
     */
    public function checkEndpoint($endpoint, $isCName)
    {
        $ret_endpoint = null;
